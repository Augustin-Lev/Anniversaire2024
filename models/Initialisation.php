    ...
    <?php
        ...
        function fillDB($NameDB,$UserDB,$passwordDB,$name,$surname,$email,$password){
            ...
            $sql ='INSERT INTO salaries (statut, nom, prenom, email, motDePasse) VALUES ( :statut, :nom, :prenom, :email, :motDePasse);';
                ...
                $pdoStatement->bindValue(':statut',"Gérant", PDO::PARAM_STR); // remplacement des données de la variable $sql
                $pdoStatement->bindValue(':nom',$name,PDO::PARAM_STR);        // (aurait pu être remplacé dans $sql directement )
                $pdoStatement->bindValue(':prenom',$surname,PDO::PARAM_STR);
                $pdoStatement->bindValue(':email',$email,PDO::PARAM_STR);
                $pdoStatement->bindValue(':motDePasse',password_hash($password,PASSWORD_DEFAULT),PDO::PARAM_STR); //hash du mot de passe pour plus de sécurité
